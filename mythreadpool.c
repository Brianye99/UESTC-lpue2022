...
threadpool_t *threadpool_create(int min_thr_num, int max_thr_num, int queue_max_size)
{
	...
	do {
		if ((pool = (threadpool_t *)malloc(sizeof(threadpool_t))) == NULL) {
			printf("malloc threadpool fail");	//线程池创建失败打印
			break;/*跳出do while*/
		}
		...
		if (pool->threads == NULL) {
			printf("malloc threads fail");	//线程IDmalloc失败打印
			break;
		}
		memset(pool->threads, 0, sizeof(pthread_t)*max_thr_num);	//初始化内存储存线程ID

		/* 队列开辟空间 */
		pool->task_queue = (threadpool_task_t *)malloc(sizeof(threadpool_task_t)*queue_max_size); //每个元素都是一个结构体，结构体中有一个函数指针和一个void*的指针。
		if (pool->task_queue == NULL) {
			printf("malloc task_queue fail");	//队列内存malloc失败打印
			break;
		}
		...
		{
			printf("init the lock or cond fail");	//初始化互斥锁和条件变量失败打印
			break;
		}
...
void *threadpool_thread(void *threadpool)
{
	...
	while (true)
	{
		...
		pool->queue_size--;													//达到总容量的时候取余等于0回到起点
...
{
	printf("pool inited\n");
	threadpool_t *thp = threadpool_create(10, 100, 100);/*创建线程池，池里最小3个线程，最大100，队列最大100*/
	...
	for (int i = 0; i < 1000; i++)
	{
		...
		threadpool_add(thp, process, list);     /* 向任务队列中添加任务 */
